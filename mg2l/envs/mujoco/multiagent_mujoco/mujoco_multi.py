...
class MujocoMulti(MultiAgentEnv):
    ...
    def __init__(self, batch_size=None, **kwargs):
        ...
        self.share_observation_space = [Box(low=np.array([-10] * (self.obs_size * self.n_agents), dtype=np.float32),
                                            high=np.array([10] * (self.obs_size * self.n_agents), dtype=np.float32),
                                            dtype=np.float32) for _ in range(self.n_agents)]
        pass
